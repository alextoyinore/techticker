...
**Instructions:**
1.  **Data Structure:** The widget will be populated with an array of article objects. Each article has these properties: \`id\`, \`title\`, \`excerpt\`, \`featuredImage\`, \`url\` (which is pre-formatted as '/article/{id}'), \`authorName\`, and \`createdAt\`.
...
3.  **Placeholders:** Inside the loop, use Handlebars-style placeholders to inject article data: \`{{title}}\`, \`{{excerpt}}\`, \`{{featuredImage}}\`, \`{{url}}\`, \`{{authorName}}\`, and \`{{createdAt}}\`.
