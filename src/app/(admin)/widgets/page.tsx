...
export default function WidgetsPage() {
    ...
    const handleEditClick = (widget: Widget) => {
        ...
        setSelectedTemplateId(null); // No easy way to know if it came from a template
        if (widget.config) {
    ...
    const handleGenerateHtml = async () => {
        ...
        try {
            ...
            setSelectedTemplateId(null); // Custom HTML, so deselect template
            ...
            <Dialog open={isDialogOpen} onOpenChange={handleDialogOpenChange}>
                ...
                <DialogContent className="sm:max-w-4xl">
                    <form onSubmit={handleSaveWidget}>
                        ...
                        <DialogDescription>
                            First, give your widget a name. Then, choose a template or generate the HTML with AI.
                        </DialogDescription>
                        </DialogHeader>
                            ...
                            <div className="grid gap-6 py-4">
                                ...
                                <Accordion type="single" collapsible className="w-full" defaultValue="item-1">
                                    <AccordionItem value="item-1">
                                        ...
                                        <AccordionContent>
                                            <RadioGroup value={selectedTemplateId ?? ""} onValueChange={handleTemplateSelect} className="grid grid-cols-2 lg:grid-cols-3 gap-4 py-4">
                                                {widgetTemplates.map((template) => (
                                                    <div key={template.id}>
                                                        ...
                                                        <Label
                                                            htmlFor={template.id}
                                                            className={cn(
                                                                "flex h-full flex-col items-center justify-between rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary",
                                                                "cursor-pointer"
                                                            )}
                                                        >
                                                            <div className="h-24 w-full bg-background rounded-lg p-2 border mb-2">
                                                                {template.preview}
                                                            </div>
                                    ...
                                    <AccordionItem value="item-2">
                                        <AccordionTrigger>Or Generate Custom HTML with AI</AccordionTrigger>
                                        <AccordionContent>
                                            <div className="space-y-2 pt-4">
                                                <Label htmlFor="description">Describe the widget layout</Label>
                                                <Textarea id="description" value={widgetDescription} onChange={(e) => setWidgetDescription(e.target.value)} placeholder="e.g., A 3-column grid of cards with a large image, title, and excerpt." />
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="html">Widget HTML</Label>
                                    <Textarea id="html" value={widgetHtml} onChange={(e) => setWidgetHtml(e.target.value)} placeholder="Select a template or generate with AI to see HTML here." rows={8} className="font-mono text-xs" required />
